...
internal fun assetOptions(fallbackLabel: String, defaults: List<String>?, custom: String?): List<Pair<String, String>> {
    val items = mutableListOf("" to fallbackLabel)
    defaults.orEmpty().filter { it.isNotBlank() }.forEach { value -> items += value to assetLabel(value) }
    custom?.takeIf { it.isNotBlank() }?.let { items += it to CustomAssetLabel }
    return items.distinctBy { it.first }
}

internal fun assetLabel(value: String): String = value.substringAfterLast('/').ifBlank { value }
...
internal fun resolveAssetUrl(value: String?): String? {
    ...
    return BuildConfig.STATIC_RESOURCE_BASE_URL.trimEnd('/') + path
}
